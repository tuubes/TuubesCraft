...
import ClientSettingsPacketBuilder._
...
final class ClientSettingsPacket(var locale: String, var viewDistance: Byte, var chatMode: Int, var chatColors: Boolean, var displayedSkinParts: Int, var mainHand: Int) extends Packet[CraftAttach] {
  override def write(out: NiolOutput): Unit = {
    out.putVarstring(locale, UTF_8)
    out.putByte(viewDistance)
    out.putVarint(chatMode)
    out.putBool(chatColors)
    out.putInt(displayedSkinParts)
    out.putVarint(mainHand)
  }
	
  override def obj = ClientSettingsPacket

	
}
object ClientSettingsPacket extends PacketObj[CraftAttach, ClientSettingsPacket] {
  override val id = 4
	
  override def read(in: NiolInput): ClientSettingsPacket = {
    val locale = in.getVarstring(UTF_8)
    val viewDistance = in.getByte()
    val chatMode = in.getVarint()
    val chatColors = in.getBool()
...
final class ClientSettingsPacketBuilder[P <: ClientSettingsPacket] extends PacketBuilder[ClientSettingsPacket, P =:= Complete] {
...
object ClientSettingsPacketBuilder {
  ...
  type Complete = ClientSettingsPacket with Locale with ViewDistance with ChatMode with ChatColors with DisplayedSkinParts with MainHand
}

...
final class WorldBorderPacket(var action: Int, var diameter: Double, var oldDiameter: Double, var newDiameter: Double, var speed: Long, var x: Double, var z: Double, var x: Double, var z: Double, var oldDiameter: Double, var newDiameter: Double, var speed: Long, var portalTeleportBoundary: Int, var warningTime: Int, var warningBlocks: Int, var warningTime: Int, var warningBlocks: Int) extends Packet {
  ...
  override def id = WorldBorderPacket.id

	
}
object WorldBorderPacket extends PacketObj[WorldBorderPacket] {
  override val id = 55
	
  override def read(in: NiolOutput): WorldBorderPacket {
...
final class WorldBorderPacketBuilder[P <: WorldBorderPacket] extends PacketBuilder[WorldBorderPacket, Complete] {
...
object WorldBorderPacketBuilder {
  ...
  sealed trait WarningTime
  sealed trait WarningBlocks
  type Complete = WorldBorderPacketBuilder with Action with Diameter with OldDiameter with NewDiameter with Speed with X with Z with X with Z with OldDiameter with NewDiameter with Speed with PortalTeleportBoundary with WarningTime with WarningBlocks with WarningTime with WarningBlocks
}

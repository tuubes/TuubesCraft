...
final class SetSlotPacket(var windowId: Byte, var slot: Short, var slotData: AnyRef) extends Packet {
  ...
  override def id = SetSlotPacket.id

	
}
object SetSlotPacket extends PacketObj[SetSlotPacket] {
  override val id = 22
	
  override def read(in: NiolOutput): SetSlotPacket {
...
final class SetSlotPacketBuilder[P <: SetSlotPacket] extends PacketBuilder[SetSlotPacket, Complete] {
...
object SetSlotPacketBuilder {
  ...
  type Complete = SetSlotPacketBuilder with WindowId with Slot with SlotData
}

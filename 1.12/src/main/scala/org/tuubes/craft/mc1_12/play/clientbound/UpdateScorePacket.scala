...
import UpdateScorePacketBuilder._
import org.tuubes.core.network._
import org.tuubes.craft.CraftAttach

/** Packet class auto-generated by DataTractor */
final class UpdateScorePacket(var entityName: String, var action: Byte, var objectiveName: String, var value: Option[Int]) extends Packet[CraftAttach] {
  override def write(out: NiolOutput): Unit = {
    out.putVarstring(entityName, UTF_8)
    out.putByte(action)
    out.putVarstring(objectiveName, UTF_8)
    if (value.isDefined) {
      out.putVarint(value.get)
    }
  }
	
  override def obj = UpdateScorePacket

	
}
object UpdateScorePacket extends PacketObj[CraftAttach, UpdateScorePacket] {
  override val id = 69
	
  override def read(in: NiolInput): UpdateScorePacket = {
    val entityName = in.getVarstring(UTF_8)
    val action = in.getByte()
    val objectiveName = in.getVarstring(UTF_8)
    // TODO read value
    new UpdateScorePacket(entityName, action, objectiveName, null)
  }
}
/** Packet builder auto-generated by DataTractor */
final class UpdateScorePacketBuilder[P <: UpdateScorePacket] extends PacketBuilder[UpdateScorePacket, P =:= Complete] {
...
object UpdateScorePacketBuilder {
  ...
  sealed trait Value
  type Complete = UpdateScorePacket with EntityName with Action with ObjectiveName with Value
}

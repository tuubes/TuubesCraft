...
final class ParticlePacket(var particleId: Int, var longDistance: Boolean, var x: Float, var y: Float, var z: Float, var offsetX: Float, var offsetY: Float, var offsetZ: Float, var particleData: Float, var data: Array[Int]) extends Packet {
  override def write(out: NiolOutput): Unit {
    out.putInt(particleId)
    out.putBoolean(longDistance)
    out.putFloat(x)
    out.putFloat(y)
    out.putFloat(z)
  ...
  override def id = ParticlePacket.id

	
}
object ParticlePacket extends PacketObj[ParticlePacket] {
  override val id = 34
	
  override def read(in: NiolOutput): ParticlePacket {
    val particleId = in.getInt()
    val longDistance = in.getBoolean()
    val x = in.getFloat()
    val y = in.getFloat()
    val z = in.getFloat()
...
final class ParticlePacketBuilder[P <: ParticlePacket] extends PacketBuilder[ParticlePacket, Complete] {
...
object ParticlePacketBuilder {
  ...
  sealed trait Data
  type Complete = ParticlePacketBuilder with ParticleId with LongDistance with X with Y with Z with OffsetX with OffsetY with OffsetZ with ParticleData with Data
}

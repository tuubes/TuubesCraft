...
final class ExplosionPacket(var x: Float, var y: Float, var z: Float, var radius: Float, var records: Array[???], var playerMotionX: Float, var playerMotionY: Float, var playerMotionZ: Float) extends Packet {
  ...
  override def id = ExplosionPacket.id

	
}
object ExplosionPacket extends PacketObj[ExplosionPacket] {
  override val id = 28
	
  override def read(in: NiolOutput): ExplosionPacket {
    val x = in.getFloat()
    val y = in.getFloat()
    val z = in.getFloat()
...
final class ExplosionPacketBuilder[P <: ExplosionPacket] extends PacketBuilder[ExplosionPacket, Complete] {
...
object ExplosionPacketBuilder {
  ...
  type Complete = ExplosionPacketBuilder with X with Y with Z with Radius with Records with PlayerMotionX with PlayerMotionY with PlayerMotionZ
}

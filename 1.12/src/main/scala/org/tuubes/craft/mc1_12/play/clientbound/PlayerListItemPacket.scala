...
object PlayerListItemPacket extends PacketObj[CraftAttach, PlayerListItemPacket] {
  override val id = 46
	
  override def read(in: NiolInput): PlayerListItemPacket = {
    ...
    val isSigned = in.getBool()
    // TODO read signature
    val gamemode = in.getVarint()
    val ping = in.getVarint()
    val hasDisplayName = in.getBool()
...
final class PlayerListItemPacketBuilder[P <: PlayerListItemPacket] extends PacketBuilder[PlayerListItemPacket, P =:= Complete] {
...
object PlayerListItemPacketBuilder {
  ...
  type Complete = PlayerListItemPacket with Action with NumberOfPlayers with Uuid with Name with NumberOfProperties with Name with Value with IsSigned with Signature with Gamemode with Ping with HasDisplayName with DisplayName with Gamemode with Ping with HasDisplayName with DisplayName with NoFields
}

...
final class BlockChangePacket(var location: Long, var blockId: Int) extends Packet {
  override def write(out: NiolOutput): Unit {
    out.putLong(location)
    out.putVarint(blockId)
  }
	
  override def id = BlockChangePacket.id
...
object BlockChangePacket extends PacketObj[BlockChangePacket] {
  override val id = 11
	
  override def read(in: NiolOutput): BlockChangePacket {
...
final class BlockChangePacketBuilder[P <: BlockChangePacket] extends PacketBuilder[BlockChangePacket, Complete] {
...
object BlockChangePacketBuilder {
  ...
  type Complete = BlockChangePacketBuilder with Location with BlockId
}

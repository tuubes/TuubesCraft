...
final class ChunkDataPacket(var chunkX: Int, var chunkZ: Int, var groundUpContinuous: Boolean, var primaryBitMask: Int, var data: Array[Byte], var blockEntities: Array[TagCompound]) extends Packet {
  override def write(out: NiolOutput): Unit = {
    out.putInt(chunkX)
    out.putInt(chunkZ)
    out.putBoolean(groundUpContinuous)
...
object ChunkDataPacket extends PacketObj[ChunkDataPacket] {
  ...
  override def read(in: NiolOutput): ChunkDataPacket = {
    ...
    val groundUpContinuous = in.getBoolean()
...
object ChunkDataPacketBuilder {
  ...
  type Complete = ChunkDataPacketBuilder with ChunkX with ChunkZ with GroundUpContinuous with PrimaryBitMask with Data with BlockEntities
}

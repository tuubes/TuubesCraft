...
final class EntityPropertiesPacket(var entityId: Int, var numberOfProperties: Int, var key: String, var value: Double, var modifiers: Array[???]) extends Packet {
  ...
  override def id = EntityPropertiesPacket.id

	
}
object EntityPropertiesPacket extends PacketObj[EntityPropertiesPacket] {
  override val id = 77
	
  override def read(in: NiolOutput): EntityPropertiesPacket {
...
final class EntityPropertiesPacketBuilder[P <: EntityPropertiesPacket] extends PacketBuilder[EntityPropertiesPacket, Complete] {
...
object EntityPropertiesPacketBuilder {
  ...
  type Complete = EntityPropertiesPacketBuilder with EntityId with NumberOfProperties with Key with Value with Modifiers
}

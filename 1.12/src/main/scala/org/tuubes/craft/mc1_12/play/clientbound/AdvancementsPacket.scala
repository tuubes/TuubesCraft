...
final class AdvancementsPacket(var resetOrClear: Boolean, var mappingSize: Int, var key: String, var value: ???, var identifiers: Array[String], var progressSize: Int, var key: String, var value: ???) extends Packet {
...
object AdvancementsPacketBuilder {
  ...
  sealed trait Key
  sealed trait Value
  type Complete = AdvancementsPacketBuilder with ResetOrClear with MappingSize with Key with Value with Identifiers with ProgressSize with Key with Value
}

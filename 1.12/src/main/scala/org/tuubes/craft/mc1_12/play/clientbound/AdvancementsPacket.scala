...
final class AdvancementsPacket(var reset/clear: Boolean, var mappingSize: Int, var key: String, var value: ???, var identifiers: Array[String], var progressSize: Int, var key: String, var value: ???) extends Packet {
  override def write(out: NiolOutput): Unit {
    out.putBoolean(reset/clear)
  ...
  override def id = AdvancementsPacket.id

	
}
object AdvancementsPacket extends PacketObj[AdvancementsPacket] {
  override val id = 76
	
  override def read(in: NiolOutput): AdvancementsPacket {
    val reset/clear = in.getBoolean()
    ...
    val key = in.getVarstring(UTF_8)
    // TODO read value
    new AdvancementsPacket(reset/clear, mappingSize, key, value, identifiers, progressSize, key, value)
  }
}
/** Packet builder auto-generated by DataTractor */
final class AdvancementsPacketBuilder[P <: AdvancementsPacket] extends PacketBuilder[AdvancementsPacket, Complete] {
  private[this] var reset/clear: Boolean = _
  ...
  def withReset/Clear(reset/clear: Boolean): AdvancementsPacketBuilder[P with Reset/Clear] = {
    this.reset/clear = reset/clear
    this.asInstanceOf[AdvancementsPacketBuilder[P with Reset/Clear]]
  }
...
object AdvancementsPacketBuilder {
  ...
  sealed trait Key
  sealed trait Value
  type Complete = AdvancementsPacketBuilder with Reset/Clear with MappingSize with Key with Value with Identifiers with ProgressSize with Key with Value
}

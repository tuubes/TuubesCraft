...
final class PlayerPositionAndLookPacket(var x: Double, var y: Double, var z: Double, var yaw: Float, var pitch: Float, var flags: Byte, var teleportId: Int) extends Packet {
  ...
  override def obj = PlayerPositionAndLookPacket

	
}
object PlayerPositionAndLookPacket extends PacketObj[PlayerPositionAndLookPacket] {
  override val id = 46
	
  override def read(in: NiolOutput): PlayerPositionAndLookPacket = {
    val x = in.getDouble()
    val y = in.getDouble()
    val z = in.getDouble()
    val yaw = in.getFloat()
...
final class PlayerPositionAndLookPacketBuilder[P <: PlayerPositionAndLookPacket] extends PacketBuilder[PlayerPositionAndLookPacket, Complete] {
...
object PlayerPositionAndLookPacketBuilder {
  ...
  type Complete = PlayerPositionAndLookPacketBuilder with X with Y with Z with Yaw with Pitch with Flags with TeleportId
}
